...
<html lang="pt-br">
	...
	<body>
		<div class="container"> <!-- afasta o conteúdo da margem esquerda -->
			...
			{
				$usuario = $_SESSION['usuarioId'];//PARA USUÁRIO ESCOLA
				echo "<h1 class='text-center'>Todos os Meus Agendamentos</h1>";
			}
			else
			{
			...
			$query = "SELECT * FROM agendamentos WHERE (escola = $usuario AND excluido = 0) LIMIT $inicio, $total_de_resultados_por_pagina"; // cada clique no link, executa este select uma vez.
			...
			$query = "SELECT COUNT(id) AS total_agendamentos FROM agendamentos WHERE escola = $usuario"; // conta todos os usuarios na tabela e retorna o total
			$result_query = mysqli_query($conn, $query);
			$row_pagina = mysqli_fetch_assoc($result_query); //echo $row_pagina['total_agendamentos'];			
			$total_de_paginas = ceil($row_pagina['total_agendamentos'] / $total_de_resultados_por_pagina); //determina o total de paginas de resultados para suportar a exibição de todos os usuários que foram contados na tabela
