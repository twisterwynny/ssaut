...
<html lang="pt-br">
	...
	<body>
		<div class="container"> <!-- afasta o conteúdo da margem esquerda -->
			...
			$query = "SELECT * FROM exposicoes LIMIT $inicio, $total_de_resultados_por_pagina"; // cada clique no link, executa este select uma vez.
			...
			while($row = mysqli_fetch_assoc($result_query))
			{
				...
				//echo "<a href='proc-excluir-exposicao.php?id=" . $row['id'] . "' data-confirm='Esta operação não pode ser desfeita! Confirmar exclusão deste Usuário?'>excluir</a><br><hr>";
				echo "<a href='proc-excluir-exposicao.php?id=" . $row['id'] . "' data-confirm=''>excluir</a><br><hr>";
			}			
			
			$query = "SELECT COUNT(id) AS total_exposicoes FROM exposicoes"; // conta todos os Exposições na tabela e retorna o total
			$result_query = mysqli_query($conn, $query);
			$row_pagina = mysqli_fetch_assoc($result_query);
			//echo $row_pagina['total_exposicoes'];			
			$total_de_paginas = ceil($row_pagina['total_exposicoes'] / $total_de_resultados_por_pagina); //determina o total de paginas de resultados para suportar a exibição de todos os usuários que foram contados na tabela
			
			$max_links = 2; // define o limite de links exibidos em "PAGINAS ANTERIORES" e "PAGINAS SEGUINTES"	

			echo "<a href='form-listar-exposicoes.php?pagina=1'>Primeira</a> ";
			
			for($pagina_anterior = $pagina - $max_links; $pagina_anterior <= $pagina - 1; $pagina_anterior++)
			{
				if($pagina_anterior >= 1)
				{					
					echo "<a href='form-listar-exposicoes.php?pagina=$pagina_anterior'>$pagina_anterior</a> ";
				}
			}
			...
			for($pagina_seguinte = $pagina + 1; $pagina_seguinte <= $pagina + $max_links; $pagina_seguinte++)
			{
				...
				{					
					echo "<a href='form-listar-exposicoes.php?pagina=$pagina_seguinte'>$pagina_seguinte</a> ";
				}
			}			
			
			echo "<a href='form-listar-exposicoes.php?pagina=$total_de_paginas'>Ultima</a>";
		...
		<script>
			$(document).ready(function()
			{
				$('a[data-confirm]').click(function(ev)
				{
					...
					{
						$('body').append('<div class="modal fade" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="modal-confirmar-excluir" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header bg-danger text-white">EXCLUIR EXPOIÇÃO<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button></div><div class="modal-body">Esta operação não pode ser desfeita! Confirmar exclusão desta Exposição?</div><div class="modal-footer"><button type="button" class="btn btn-primary" data-dismiss="modal">NÃO! deixar como está</button><a class="btn btn-danger text-white" id="dataComfirmOK">SIM! Excluir</a></div></div></div></div>');
					}
